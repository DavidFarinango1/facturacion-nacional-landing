...
' + (rows ? '<div class="table-wrap"><table class="tbl tbl--compact"><thead><tr><th>Empresa / contacto</th><th>Correo / teléfono</th><th class="num">Pauta</th><th class="num">Ahorro anual</th><th>Estado</th><th>Fecha</th><th></th></tr></thead><tbody>' + rows + '</tbody></table></div>' : '<p class="empty">No hay leads que coincidan.</p>') + '</section>';
  ...
  VIEWS.companies = function () {
    ...
    const rows = comps.map(function (c) {
      ...
      return '<tr><td><strong>' + esc(c.company) + '</strong><div class="muted">' + (c.contact ? esc(c.contact) + ' · ' : '') + 'RUC ' + esc(c.ruc) + '</div></td><td>' + esc(c.email) + '<div class="muted">' + esc(c.phone) + '</div></td><td>' + esc(c.city) + '</td><td class="num">' + fmt(c.balance) + '</td><td>' + pill("company", c.status) + '</td><td class="muted">' + fdate(c.createdAt) + '</td><td class="actions">' + actions + '<button class="pbtn pbtn--sm" data-comp-detail="' + c.id + '">Detalle</button></td></tr>';
    }).join("");
...
<div class="pform__actions" style="align-self:end"><button type="submit" class="pbtn pbtn--primary">Crear acceso</button><span id="adminMsg" class="pmsg"></span></div></form></section>';
  ...
  function bind() {
    ...
    view.querySelectorAll("[data-comp-detail]").forEach(function (b) { b.addEventListener("click", function () {
      ...
      openDrawer(c.company, '<dl class="dl"><dt>Estado</dt><dd>' + pill("company", c.status) + '</dd><dt>Contacto</dt><dd>' + (c.contact ? esc(c.contact) : '<span class="muted">no indicado</span>') + '</dd><dt>RUC</dt><dd>' + esc(c.ruc) + '</dd><dt>Correo</dt><dd>' + esc(c.email) + '</dd><dt>Teléfono</dt><dd>' + esc(c.phone) + '</dd><dt>Ciudad</dt><dd>' + esc(c.city) + '</dd><dt>Saldo de pauta</dt><dd>' + fmt(c.balance) + '</dd><dt>Pauta asignada</dt><dd>' + fmt(spentTotal) + ' en ' + spends.length + ' campaña(s)</dd><dt>Registro</dt><dd>' + fdatetime(c.createdAt) + '</dd></dl>\
